...
static millis_t oldtime;
static double acc0_offset_x_par1;
static double acc0_offset_x_par2;
static double acc0_offset_x_par3;
static double acc_xyz_0_old;
static double speed;
static uint8_t init_meter;
 ...
 * */
//void
//mpu9150_compass_setup_bypass (void) // Pass through mode, MCU directly accesses Ak8975c, doens't work with my sensor, possible hardware fault
//{
//volatile uint8_t buffer[2];
//
...
xyz mpu9150_read_acc_xyz_0(void)
{
	...
	acc.x = (double)(((buffer[1] + (buffer[0] << 8)) / acc_sensitivity) - 0.02319);
	acc.y = (double)(((buffer[3] + (buffer[2] << 8)) / acc_sensitivity) - 0.00138);
	acc.z = (double)(((buffer[5] + (buffer[4] << 8)) / acc_sensitivity) + 0.05077);
...
xyz mpu9150_read_gyro_xyz_0(void)
{
	...
	gyro.x = (float)((((buffer[0] << 8) + buffer[1]) / gyro_sensitivity) - 35.2);
	gyro.y = (float)((((buffer[2] << 8) + buffer[3]) / gyro_sensitivity) - 1.14);
...
xyz mpu9150_get_acc_angles_0()
{
	...
	angle.x = (180 / pi) * (atan2(acc.x, sqrt(acc.y * acc.y + acc.z * acc.z))); //roll

	angle.y = (180 / pi) * (atan2(sqrt(acc.x * acc.x + acc.z * acc.z), acc.y)) - 90; //pitch
...
xyz mpu9150_get_gyro_angles_0(xyz oldangle)
{
	...
	newangle.x = (initangle.x * tick_counter * 0.000001) * 2 + oldangle.x;
	newangle.y = (initangle.y * tick_counter * 0.000001) * 2 + oldangle.y;
	tick_counter = 0;
...
void mpu9150_calibrate_acc_0(void)
{
	xyz acc_vect;
	xyz acc_avrg_vect;
	double acc0_offset_x_angle;

	speed = 0;
	for (uint16_t i = 0; i < 1500; i++)
	{
		acc_vect = mpu9150_read_acc_xyz_0();
		acc_avrg_vect.x = acc_vect.x;
		acc_avrg_vect.y = acc_vect.y;
		acc_avrg_vect.z = acc_vect.z;
		_delay_us(60);
	}

	acc_avrg_vect.x /= 1500;
	acc_avrg_vect.y /= 1500;
	acc_avrg_vect.z /= 1500;

	acc0_offset_x_angle = (180 / pi) * (atan2(acc_vect.x, sqrt(acc_vect.y * acc_vect.y + acc_vect.z * acc_vect.z))); //pitch

	if (acc0_offset_x_angle > 0)
	{
		acc0_offset_x_par2 = acc0_offset_x_angle / (90 - acc0_offset_x_angle);
		acc0_offset_x_par1 = acc_avrg_vect.x;
	}
	else
	{
		acc0_offset_x_par2 = -acc0_offset_x_angle / (90 + acc0_offset_x_angle);
	...
	serialWrite_newline();
	serialWrite_double(acc0_offset_x_angle, 3);
	serialWrite(" par1: ");
	serialWrite_double(acc0_offset_x_par1, 3);
	serialWrite(" par2: ");
	serialWrite_double(acc0_offset_x_par2, 3);
	serialWrite_newline();
}

double mpu9150_get_acc_distance_0(void)
{
	millis_t newtime;
	double DT;
	...
	acc_xyz_0 = mpu9150_read_acc_xyz_0().x;
	acc_xyz_0 += acc0_offset_x_par1;
	// 			acc_xyz_0+=(acc_xyz_0*acc0_offset_x_par2);  //add offset parameters
	acc_xyz_0 = (acc_xyz_0_old * 0.8) + (acc_xyz_0 * 0.2); //LPF
	acc_xyz_0_old = acc_xyz_0;
	...
	{
		newtime = millis_get();
		DT = (newtime - oldtime) * 0.001; //DT in seconds
		oldtime = newtime;
	}
	...
	serialWrite_double(acc_xyz_0, 3);
	serialWrite("   ");
	serialWrite_double(speed, 3);
	serialWrite_newline();
	return distance;
}

void mpu9150_reset_distance_meter(void)
{
	init_meter = 0;
}
